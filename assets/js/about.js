...
import { t } from './i18n.js?v=afaaa0354b';
...
import { scrollToEl } from './smooth-scroll.js?v=afaaa0354b';
...
function mountNetwork(net) {
  ...
  const start = () => {
    if (started) return;
    started = true;
    import('./network.js?v=afaaa0354b').then(({ initNetwork }) =>
      initNetwork(canvas, { tip: document.getElementById('net-tip'), data: net })
    ).then((api) => {
      if (!api) { stage.classList.add('is-flat'); return; }
      stage.classList.add('is-live');
      import('./motion.js?v=afaaa0354b').then(({ whileVisible }) => whileVisible(stage, api.play, api.hold));
