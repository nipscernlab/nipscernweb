...
import { t } from './i18n.js?v=3fc06b9482';
...
import { scrollToEl } from './smooth-scroll.js?v=3fc06b9482';
...
function mountNetwork(net) {
  ...
  const start = () => {
    if (started) return;
    started = true;
    import('./network.js?v=3fc06b9482').then(({ initNetwork }) =>
      initNetwork(canvas, { tip: document.getElementById('net-tip'), data: net })
    ).then((api) => {
      if (!api) { stage.classList.add('is-flat'); return; }
      stage.classList.add('is-live');
      import('./motion.js?v=3fc06b9482').then(({ whileVisible }) => whileVisible(stage, api.play, api.hold));
