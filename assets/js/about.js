...
import { t } from './i18n.js?v=7a8294c79a';
...
import { scrollToEl } from './smooth-scroll.js?v=7a8294c79a';
...
function mountNetwork(net) {
  ...
  const start = () => {
    if (started) return;
    started = true;
    import('./network.js?v=7a8294c79a').then(({ initNetwork }) =>
      initNetwork(canvas, { tip: document.getElementById('net-tip'), data: net })
    ).then((api) => {
      if (!api) { stage.classList.add('is-flat'); return; }
      stage.classList.add('is-live');
      import('./motion.js?v=7a8294c79a').then(({ whileVisible }) => whileVisible(stage, api.play, api.hold));
