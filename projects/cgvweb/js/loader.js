...
import { meshByKey, cellMeshesByDet, _ZERO_MAT4, _allCellIMeshes, _tileKey, _larEmKey, _hecKey } from './state.js';
  ...
  for (const mesh of meshes) {
    ...
    for (let i = 1; i < parts.length; i++) {
      if (!node.children.has(parts[i]))
        node.children.set(parts[i], { name: parts[i], parent: node, children: new Map(), meshes: [], allMeshes: [] });
...
async function _opfsSaveGz(buffer, version) {
  ...
  try {
    ...
    await wd.write(buffer); await wd.close();
    ...
    await wm.close();
  } catch { /* OPFS quota or transient error — best-effort */ }
}
...
export async function initScene({ setStatus, atlasMat, onSceneReady, onAtlasReady }) {
    ...
    if (cachedGz) {
      try {
        buffer = await _decompressGz(
          new Blob([cachedGz]).stream(), cachedGz.byteLength,
    ...
    try {
      ...
      if (version) {
        new Response(streamForCache).arrayBuffer()
          .then(gz => _opfsSaveGz(gz, version)).catch(() => {});
      } else {
  ...
  new GLTFLoader().parse(buffer, './', ({ scene: g }) => {
    const ghosts      = [];
    const atlasMeshes = [];
    ...
    for (const gh of ghosts) {
      gh.matrixAutoUpdate = false; gh.frustumCulled = false; gh.visible = false;
      ghostMeshByName.set(gh.name, gh);
      scene.add(gh);
    }
    for (const o of loose) {
      o.matrixAutoUpdate = false; o.frustumCulled = false; o.visible = false;
      scene.add(o);
    }
